...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-900">
      {/* Header */}
      <header className="bg-gray-800/80 backdrop-blur-xl border-b border-white/5 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white leading-none mb-1">AgroTech</h1>
        ...
        <div className="w-10 h-10 rounded-full bg-gray-700 border border-white/5 flex items-center justify-center text-emerald-400">
          <User size={20} />
        </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
        ...
        <div className="bg-gray-800 rounded-3xl p-8 border border-white/5">
          ...
          <form onSubmit={handleSoilSubmit} className="space-y-6">
            ...
            <div className="p-5 rounded-2xl bg-gray-700 border border-white/5 hover:border-emerald-500/20 transition-all">
              <div className="flex justify-between items-center mb-4">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500/50">Soil Chemistry</span>
              <span className={cn(
              ...
              <input 
                ...
                setErrors((prev: typeof errors) => ({ ...prev, ph: error }));
                setSoilData({...soilData, ph: val});
                }}
                className={cn(
                "w-full bg-gray-600 rounded-xl py-3 px-4 text-white text-sm transition-all border border-transparent",
            ...
            <div className="p-5 rounded-2xl bg-gray-700 border border-white/5 hover:border-emerald-500/20 transition-all">
              <div className="flex justify-between items-center mb-4">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500/50">Moisture Content</span>
              <button 
                type="button"
              ...
              {!showHumidityPredictor ? (
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-xs font-bold text-emerald-400">{soilData.humidity}%</span>
                </div>
                <input 
                  ...
                  setErrors((prev: typeof errors) => ({ ...prev, humidity: error }));
                ...
                <input 
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPredictionData({...predictionData, location: e.target.value})}
                  className="w-full bg-gray-600 rounded-xl py-2 px-3 text-xs text-white focus:outline-none focus:border-emerald-500/30"
                />
                <input 
                  type="text"
                  placeholder="What are neighbors growing?"
                  value={predictionData.neighborCrops}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPredictionData({...predictionData, neighborCrops: e.target.value})}
                  className="w-full bg-gray-600 rounded-xl py-2 px-3 text-xs text-white focus:outline-none focus:border-emerald-500/30"
                />
                <input 
                  type="text"
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPredictionData({...predictionData, currentWeather: e.target.value})}
                  className="w-full bg-gray-600 rounded-xl py-2 px-3 text-xs text-white focus:outline-none focus:border-emerald-500/30"
                />
                <button 
                  type="button"
                  ...
                  className="w-full bg-emerald-600/20 text-emerald-400 font-bold text-[10px] py-2 rounded-lg border border-emerald-500/20 hover:bg-emerald-600/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                >
            ...
            <div className="p-5 rounded-2xl bg-gray-700 border border-white/5">
              <label className="block">
              ...
              <select 
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSoilData({...soilData, season: e.target.value})}
                className="block w-full rounded-xl border border-transparent bg-gray-600 text-sm py-3 px-4 text-white focus:border-emerald-500/30 transition-all outline-none"
              >
            ...
            <div className="p-5 rounded-2xl bg-gray-700 border border-white/5">
              ...
              <select 
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSoilData({...soilData, water: e.target.value})}
                className="block w-full rounded-xl border border-transparent bg-gray-600 text-sm py-3 px-4 text-white focus:border-emerald-500/30 transition-all outline-none"
              >
                <option>Scant</option>
            ...
            <label className="flex items-center gap-4 cursor-pointer group p-4 rounded-2xl bg-gray-700/50 border border-white/5 hover:bg-gray-700 transition-all">
              <div className="relative flex items-center justify-center">
              <input 
                type="checkbox" 
                checked={soilData.isBeginner}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSoilData({...soilData, isBeginner: e.target.checked})}
            ...
            >
              <label className="block">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400 mb-3 block">Land Size</span>
              <div className="flex gap-2">
                <div className="relative flex-1">
                <input 
                  type="number" 
                  value={soilData.landSize}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSoilData({...soilData, landSize: e.target.value})}
                  className="block w-full rounded-xl border border-transparent bg-gray-600 text-sm py-4 px-5 pr-12 text-white focus:border-emerald-500/30 transition-all outline-none"
                  placeholder="e.g. 2.5"
                />
                <span className="absolute right-5 top-1/2 -translate-y-1/2 text-xs font-bold text-emerald-500/50">
                  {soilData.landUnit === 'Hectares' ? 'ha' : 'ac'}
                </span>
                </div>
                <select 
                value={soilData.landUnit}
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSoilData({...soilData, landUnit: e.target.value})}
                className="bg-gray-600 border border-transparent rounded-xl text-xs font-bold text-white px-3 focus:border-emerald-500/30 transition-all outline-none"
                >
          ...
          <div className="space-y-6">
            ...
            className="aspect-square rounded-3xl border-2 border-dashed border-white/10 bg-gray-700 flex flex-col items-center justify-center gap-4 cursor-pointer hover:bg-gray-600 transition-colors overflow-hidden relative group"
            >
            {image ? (
              <>
              ...
              <>
              <div className="w-16 h-16 rounded-full bg-gray-600 flex items-center justify-center text-emerald-400 shadow-lg">
                <Camera size={32} />
              </div>
            ...
            className="w-full bg-emerald-600 text-black font-black uppercase tracking-widest text-xs py-5 rounded-2xl shadow-lg shadow-emerald-500/20 hover:bg-emerald-500 hover:-translate-y-1 active:translate-y-0 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
          ...
          <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
            ...
            records.map((record: typeof records[0]) => (
              <div 
              key={record.id}
              onClick={() => setResult(record.result)}
              className="p-5 rounded-2xl border border-white/5 bg-gray-700 hover:bg-gray-600 transition-all cursor-pointer group"
              >
        ...
        <motion.div 
          ...
          animate={{ opacity: 1, y: 0 }}
          className="bg-emerald-950 rounded-3xl p-8 text-white relative overflow-hidden border border-emerald-500/10"
        >
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-bold text-emerald-400">Farming Tips</h3>
          </div>
        ...
        <div className="bg-gray-800 rounded-3xl border border-white/5 min-h-[600px] flex flex-col overflow-hidden shadow-lg"></div>
          <div className="p-6 border-b border-white/5 flex items-center justify-between bg-gray-700/50 sticky top-0 z-20">
          <div className="flex items-center gap-3">
            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <h2 className="text-xl font-bold text-white">AI Farmer Analysis</h2>
          </div>
          {result && (
            <div className="flex items-center gap-3">
            <button 
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="text-xs text-gray-500 mt-3 leading-relaxed">Analyzing soil patterns and environmental data...</p>
              </div>
            </motion.div>
            ) : result ? (
            <motion.div 
            ...
            >
              ...
              >
                ...
                <div>
                  <h3 className="text-lg font-bold text-white">Your Watering Schedule</h3>
                  <p className="text-[10px] uppercase tracking-widest text-emerald-500/60 font-black">Daily Simple Plan</p>
                </div>
                </div>
                <div className="text-sm bg-black/20 rounded-xl p-4 border border-white/5">
              ...
              >
                ...
                <div>
                  <h3 className="text-lg font-bold text-white">Personalized Farming Tips</h3>
            ...
            >
              <div className="w-24 h-24 rounded-full bg-gray-700 flex items-center justify-center text-emerald-500/20">
              <Info size={48} />
              </div>
              <div>
              <h3 className="text-xl font-bold text-white mb-3">Ready to Grow?</h3>
              <p className="text-sm text-gray-500 leading-relaxed">
                Enter your soil parameters or upload a photo to get AI-driven farming advice.
              </p>
              </div>
              <div className="grid grid-cols-2 gap-4 w-full">
              <div className="p-5 rounded-2xl bg-gray-700 border border-white/5 text-left">
          ...
          <div className="p-5 bg-gray-700/50 border-t border-white/5 text-center">
      ...
      {/* Mobile Nav */}
      <div className="md:hidden fixed bottom-6 left-6 right-6 bg-gray-800/90 backdrop-blur-xl border border-white/10 px-8 py-4 rounded-3xl flex justify-between items-center z-50 shadow-lg">
      <button 
        onClick={() => setActiveTab('soil')}
